...
ga.rin1 = inla(formula1,
               ...
               control.inla=list(control.vb = list(enable = TRUE), int.strategy = "eb"),
               family = "poisson")


if(TRUE){
  # remove.packages("fbesag")
  # library("devtools")
  # install_github("esmail-abdulfattah/fbesag")
  library("fbesag")
  cmodel_pbesag <- fbesag::get_fbesag(graph, id = id_p, sd_gamma = sd_sim, param = list(p1 = 1, p2 = 1e-5))
}else{
  source("~/Documents/KAUST/Projects/Papers R codes/partition-besag/final_version/wrapper.R")
  cmodel_pbesag <- get_pbesag(graph, id = id_p, sd_gamma = sd_sim, param = list(p1 = 1, p2 = 1e-5))
}
...
##
#---> USE "RGENERIC" IF fbesag did not work
##
...
sbesag.model <- inla.rgeneric.define(inla.rgeneric.fbesag.model, W = as.matrix(scaled_graph), id_p = id_p, scaled_cnst = scaled_cnst, sd_sim = sd_sim)

formula3 = y ~ 1 + f(id, model = sbesag.model, constr= TRUE, rankdef=1, replicate = idx.rep)

ctrc <- list(dic=TRUE, waic=TRUE, cpo=TRUE)
ga.rin3 = inla(formula3,
               data = new_data,
               verbose = TRUE,
               control.compute = ctrc,
               control.inla=list(control.vb = list(enable = TRUE), int.strategy = "eb"),
...
ga.rin3$internal.summary.hyperpar$mean
